 ...
 * be used by CS LPC and CS LP concrete use cases
 */
...
static inline void CsLppUseCaseDelete(CsLpUseCaseObject* cs_lpp_use_case) {
  CsLpUseCaseDelete(cs_lpp_use_case);
}

/**
 * @brief Get the current load control limit data
 * @param self CS LP Use Case instance to get the load control limit with
 * @param limit Buffer to load control limit data into
 * @return kEebusErrorOk on success, error code otherwise
 */
static inline EebusError CsLppGetProductionLimit(const CsLpUseCaseObject* self, LoadLimit* limit) {
  return CsLpGetLimit(self, limit);
}
...
CsLppSetProductionLimit(CsLpUseCaseObject* self, int64_t limit, int8_t scale, bool is_active, bool is_changeable) {
  return CsLpSetLimit(self, limit, scale, is_active, is_changeable);
}
 ...
 * @param self CS LP Use Case instance to get the Failsafe Limit with
...
static inline EebusError CsLppGetFailsafeProductionActivePowerLimit(
  ...
  return CsLpGetFailsafeActivePowerLimit(self, power_limit, is_changeable);
}

/**
 * @brief Set the Failsafe Limit for the produced active (real) power of the
 * Controllable System. This limit becomes activated in "init" state or "failsafe state".
 * @param self CS LP Use Case instance to set the Failsafe Limit with
...
static inline EebusError CsLppSetFailsafeProductionActivePowerLimit(
  ...
  return CsLpSetFailsafeActivePowerLimit(self, power_limit, is_changeable);
}
 ...
 * @param self CS LP Use Case instance to get the Failsafe Duration Minimum with
...
CsLppGetFailsafeDurationMinimum(const CsLpUseCaseObject* self, DurationType* duration, bool* is_changeable) {
  return CsLpGetFailsafeDurationMinimum(self, duration, is_changeable);
}

/**
 * @brief Set minimum time the Controllable System remains in "failsafe state" unless conditions
 * specified in this Use Case permit leaving the "failsafe state"
 * @param self CS LP Use Case instance to set the Failsafe Duration Minimum with
...
CsLppSetFailsafeDurationMinimum(CsLpUseCaseObject* self, const DurationType* duration, bool is_changeable) {
  return CsLpSetFailsafeDurationMinimum(self, duration, is_changeable);
}
 ...
 *
 * @param self CS LP Use Case instance to get the nominal max with
...
static inline EebusError CsLppGetProductionNominalMax(CsLpUseCaseObject* self, ScaledValue* nominal_max) {
  return CsLpGetNominalMax(self, nominal_max);
}
...
static inline EebusError CsLppSetProductionNominalMax(CsLpUseCaseObject* self, const ScaledValue* new_nominal_max) {
  return CsLpSetNominalMax(self, new_nominal_max);
}
...
static inline ElectricalConnectionCharacteristicTypeType CsLppGetElectricalConnectionCharacteristicType(
  ...
  return CsLpGetElectricalConnectionCharacteristicType(self);
}
